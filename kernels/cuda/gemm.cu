...
cudaError_t launch_column_sum(
    ...
    switch (type) {
        case CUDA_R_16F:
            column_sum_kernel<<<blocks, threads, 0, stream>>>(static_cast<const __half*>(x), static_cast<__half*>(y), m, n);
            return cudaPeekAtLastError();
        case CUDA_R_16BF:
            column_sum_kernel<<<blocks, threads, 0, stream>>>(static_cast<const __nv_bfloat16*>(x), static_cast<__nv_bfloat16*>(y), m, n);
            return cudaPeekAtLastError();
        case CUDA_R_32F:
            column_sum_kernel<<<blocks, threads, 0, stream>>>(static_cast<const float*>(x), static_cast<float*>(y), m, n);
            return cudaPeekAtLastError();
        default:
            return cudaErrorInvalidValue;
    }
...
cudaError_t gemm_backward_cuda(
    ...
    if (need_grad_bias && m != 0 && n != 0) {
        cudaError_t err = launch_column_sum(
            grad_c,
            grad_bias,
            m,
            n,
...
cudaError_t gemm_backward_cuda_ex(
    const void* grad_c,
    int dtype_grad_c,
    const void* a,
    int dtype_a,
    ...
    cudaDataType_t type_grad_c;
    cudaDataType_t type_a;
    cudaDataType_t type_b;
    cudaDataType_t type_grad_a;
    cudaDataType_t type_grad_b;
    cudaDataType_t type_grad_bias;
    if (!normalize_dtype(dtype_grad_c, &type_grad_c) ||
        !normalize_dtype(dtype_a, &type_a) ||
        !normalize_dtype(dtype_b, &type_b) ||
        !normalize_dtype(dtype_grad_a, &type_grad_a) ||
        !normalize_dtype(dtype_grad_b, &type_grad_b) ||
        !normalize_dtype(dtype_grad_bias, &type_grad_bias)) {
        return cudaErrorInvalidValue;
    }
    if (type_grad_c != type_grad_bias || type_a != type_grad_a || type_b != type_grad_b) {
        return cudaErrorInvalidValue;
    }
    if (type_grad_c != type_a || type_grad_c != type_b) {
        return cudaErrorInvalidValue;
    }

    const size_t elem_size = element_size(type_grad_c);
    size_t grad_a_elements = 0;
    size_t grad_b_elements = 0;
    ...
    if (need_grad_a) {
        if (m == 0 || k == 0 || n == 0) {
            cudaError_t err = zero_buffer_async(grad_a, grad_a_elements, element_size(type_grad_a), stream);
            if (err != cudaSuccess) {
                return err;
            }
        } else if (grad_c == nullptr || b == nullptr) {
    ...
    if (need_grad_b) {
        if (k == 0 || n == 0 || m == 0) {
            cudaError_t err = zero_buffer_async(grad_b, grad_b_elements, element_size(type_grad_b), stream);
            if (err != cudaSuccess) {
                return err;
            }
        } else if (a == nullptr || grad_c == nullptr) {
    ...
    if (need_grad_bias) {
        ...
        if (m == 0) {
            cudaError_t err = zero_buffer_async(grad_bias, n, element_size(type_grad_bias), stream);
            if (err != cudaSuccess) {
                return err;
            }
        } else if (grad_c == nullptr) {
