...
cudaError_t efla_forward_cuda(const void* k, const void* v, void* state, void* output, size_t batch_size, size_t seq_len, size_t num_heads, size_t head_dim, float beta, size_t chunk_size, cudaStream_t stream) {
    ...
    if (batch_size == 0 || num_heads == 0 || head_dim == 0 || chunk_size == 0) return cudaErrorInvalidValue;
    ...
    if (!checked_mul_size_t(num_heads, head_dim, &token_stride)) return cudaErrorInvalidConfiguration;

    size_t chunk_start = 0;
    while (chunk_start < seq_len) {
        const size_t chunk_len = min_size_t(chunk_size, seq_len - chunk_start);
        const size_t token_offset = chunk_start * token_stride;
        const __nv_bfloat16* k_chunk = static_cast<const __nv_bfloat16*>(k) + token_offset;
        const __nv_bfloat16* v_chunk = static_cast<const __nv_bfloat16*>(v) + token_offset;
        __nv_bfloat16* output_chunk = static_cast<__nv_bfloat16*>(output) + token_offset;
        efla_forward_kernel_impl<<<static_cast<unsigned int>(num_blocks_size_t), kBlockSize, shared_floats, stream>>>(k_chunk, v_chunk, static_cast<__nv_bfloat16*>(state), output_chunk, batch_size, chunk_len, num_heads, head_dim, beta);
